...
# 数据输出根目录
DATA_ROOT = REPO_ROOT / "test_density" / "datas_last" / "cpu_11"
...
def set_switch_env(switch_num: int) -> None:
    ...
    # compress_mode: 0=FP32密集, 1=COO稀疏, 2=SVD低秩
    os.environ.setdefault("compress_mode", "2")
    
    # 启用几号GPU（仅当 PYFLOW_USE_TORCH=1 时有效）
    os.environ["PYFLOW_GPU_ID"]  = "2"
...
def get_result_dir(params: dict) -> Path:
    ...
    if use_torch == "1":
        base = REPO_ROOT / "test_density" / "datas_last" / "gpu_11"
    else:
        base = DATA_ROOT
