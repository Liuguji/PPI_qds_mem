#!/usr/bin/env python3
"""
flow_imb 分支 111 测试（parallel + compress + checkpoint，GPU 批处理）。

注意：分支 111 仅支持 Torch GPU，需安装 torch + torchdiffeq。

    python test_imb/scripts/test_imb_111.py
"""
...
DATA_ROOT = REPO_ROOT / "test_imb" / "datas" / "gpu_111"
...
def set_switch_env(switch_num: int) -> None:
    ...
    os.environ["PYFLOW_GPU_ID"]     = os.environ.get("PYFLOW_GPU_ID", "2")
...
def save_result(result: dict, timestamp: str):
    ...
    serializable = {
        ...
        "branch_111": {
            ...
            "t_val": float(result["branch_111"].get("t_val", 0.0)),
            "n": int(result["branch_111"].get("n", 0)),
            "qmax": int(result["branch_111"].get("qmax", 0)),
            "lmax": float(result["branch_111"].get("lmax", 0.0)),
            "steps_evolved": result["branch_111"]["steps_evolved"],
            "ckpt_step": result["branch_111"]["ckpt_step"],
            "imbalance": result["branch_111"]["imbalance"],
            "peak_memory_bytes": result["branch_111"]["peak_memory_bytes"],
            "batch_size": result["branch_111"].get("batch_size", result["branch_111"]["n"]),
            "timing": {
                "h_diag_s": float(result["branch_111"].get("timing", {}).get("h_diag_s", 0.0)),
                "op_fwd_s": float(result["branch_111"].get("timing", {}).get("op_fwd_s", 0.0)),
                "time_evo_s": float(result["branch_111"].get("timing", {}).get("time_evo_s", 0.0)),
                "op_bck_s": float(result["branch_111"].get("timing", {}).get("op_bck_s", 0.0)),
                "total_s": float(result["branch_111"].get("timing", {}).get("total_s", 0.0)),
            },
        },
        "timestamp": timestamp,
    }
...
def run_config(config: dict) -> dict:
    ...
    imb = res111["imbalance"]
    print(f"    分支111: I(0)={imb:.4f} I(t_max)={imb:.4f}")

    return {"config": config, "branch_111": res111}
...
def main():
    ...
    try:
        ...
        if torch.cuda.is_available():
            gpu_id = int(os.environ.get("PYFLOW_GPU_ID", "3"))
            print(f"  GPU {gpu_id}: {torch.cuda.get_device_name(gpu_id)}")
    except Exception:
        pass

    print("=" * 70)
    print("  flow_imb 分支 111 测试（GPU 批处理）")
    print("=" * 70)
