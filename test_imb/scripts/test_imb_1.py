#!/usr/bin/env python3
"""
flow_imb 分支 1 测试（checkpoint，JAX CPU）。

    python test_imb/scripts/test_imb_1.py
"""
...
DATA_ROOT = REPO_ROOT / "test_imb" / "datas" / "cpu_1"
...
def save_result(result: dict, timestamp: str):
    ...
    serializable = {
        ...
        "branch_1": {
            ...
            "t_val": float(result["branch_1"].get("t_val", 0.0)),
            "n": int(result["branch_1"].get("n", 0)),
            "qmax": int(result["branch_1"].get("qmax", 0)),
            "lmax": float(result["branch_1"].get("lmax", 0.0)),
            "steps_evolved": result["branch_1"]["steps_evolved"],
            "ckpt_step": result["branch_1"]["ckpt_step"],
            "imbalance": result["branch_1"]["imbalance"],
            "peak_memory_bytes": result["branch_1"]["peak_memory_bytes"],
            "timing": {
                "h_diag_s": float(result["branch_1"].get("timing", {}).get("h_diag_s", 0.0)),
                "op_fwd_s": float(result["branch_1"].get("timing", {}).get("op_fwd_s", 0.0)),
                "time_evo_s": float(result["branch_1"].get("timing", {}).get("time_evo_s", 0.0)),
                "op_bck_s": float(result["branch_1"].get("timing", {}).get("op_bck_s", 0.0)),
                "total_s": float(result["branch_1"].get("timing", {}).get("total_s", 0.0)),
            },
        },
        "timestamp": timestamp,
    }
...
def run_config(config: dict) -> dict:
    ...
    imb = res1["imbalance"]
    print(f"    分支1: I(0)={imb:.4f} I(t_max)={imb:.4f}  ckpt={res1['ckpt_step']}")

    return {"config": config, "branch_1": res1}
...
def main():
    ...
    print("=" * 70)
    print("  flow_imb 分支 1 测试（checkpoint，JAX CPU）")
    print("=" * 70)
