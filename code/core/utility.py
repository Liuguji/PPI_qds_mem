...
def namevar(dis_type,dim,dsymm,no_state,dyn,norm,n,LIOM,species,order):
    if norm == True:
        ...
        nm = 'PT'
    # Base output directory (override with env var). The original code used /scratch paths.
    base = os.environ.get("PYFLOW_OUTDIR")
    if base is None or str(base).strip() == "":
        # Default to a local folder so the project runs out-of-the-box.
        base = os.path.abspath(os.path.join(os.getcwd(), "pyflow_out"))

    if species == 'spinful fermion':
        spec = os.path.join(base, "spin_fermion", str(dsymm))
    elif species == 'spinless fermion':
        spec = os.path.join(base, "fermion")
    ...
    if os.environ.get("PYFLOW_DISABLE_MKDIR_SLEEP", "0") not in ("1", "true", "True"):
...
def namevar3(dis_type,dim,dsymm,no_state,dyn,norm,n,LIOM,species,order):
    ...
    if species == 'spinful fermion':
        spec = os.path.join(base, "proc", "spin_fermion", str(dsymm))
    elif species == 'spinless fermion':
        spec = os.path.join(base, "proc", "fermion")
     # Make directory to store data
    if dyn == False:
        if not os.path.exists('%s/d%s/data/%s/%s/%s/O%s/%s/dataN%s' %(spec,dim,dis_type,nm,LIOM,order,'static',n)):
            os.makedirs('%s/d%s/data/%s/%s/%s/O%s/%s/dataN%s' %(spec,dim,dis_type,nm,LIOM,order,'static',n))
